...
--INSERT Tinh tri gia CTHD
CREATE TRIGGER TriGia_CTHD_INSERT
ON CTHD
FOR INSERT
AS
	DECLARE @TriGia int,@MaHD CHAR(8)
	SELECT @TriGia=Gia*SL FROM INSERTED,Ve WHERE Ve.MaVe=inserted.MaVe
	SELECT @MaHD=MaHD FROM INSERTED
	UPDATE CTHD SET TriGia=@TriGia WHERE MaHD=@MaHD
	PRINT('DA CAP NHAT TRI GIA CTHD')
